...
class MPGC { public:
	...
	PRP prp;
	MPGC(NetIOMP<nP> * io[2], ThreadPool * pool, int party, BristolFormat * cf, bool * _delta = nullptr, int ssp = 40) {
		this->party = party;
		this->io = io[0];
		this->cf = cf;
		this->ssp = ssp;
	...
	void function_independent(bool* input_value) {
		if(party != 1)
...
//		ret.get();
	...
	void function_dependent(bool * output_delta) {
		int ands = num_in;
		bool * x[nP+1];
		bool * y[nP+1];
		for(int i = 1; i <= nP; ++i) {
		...
		memcpy(output_delta, value+cf->num_wire- cf->n3, cf->n3);
	...
	void online (bool * input, bool * output_delta, bool* output_Public) {
		...
		if (party != 1) {
			io->recv_data(1, mask_input+cf->num_wire - cf->n3, (cf->n3)*sizeof(bool));
			io->flush(1);
		}
		else {
			vector<future<void>> res;
			for(int i = 2; i <= nP; ++i) {
				...
				res.push_back(pool->enqueue([this, mask_input, party2]() {
					io->send_data(party2, mask_input+cf->num_wire - cf->n3, (cf->n3)*sizeof(bool));
					io->flush(party2);
				}));
			}
			joinNclean(res);
		...
		memcpy(output_delta, value+cf->num_wire- cf->n3, cf->n3);
		memcpy(output_Public, mask_input+cf->num_wire- cf->n3, cf->n3);

		delete[] mask_input;
	}
	
	void online (bool * input, bool* output_Public) {
		bool * mask_input = new bool[cf->num_wire];
		for(int i = 0; i < num_in; ++i)
			mask_input[i] = input[i];
